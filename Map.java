...
/**
	...
	* steps to the player They player is one step away from itself its neighbors 
	* are two etc.
	* The only squares 0 steps away are walls. 
	* This allows the zombies to find their way to the player
	*/
...
@Override
/**
	* Spawns various items on the map each round 
	* it follows certain hardcoded presets for adaptive difficulty
	* one could fiddle with these settings to change the difficulty
	*/
	public void run() {
			if(zombies!=null){
	for(Zombie z: zombies){
